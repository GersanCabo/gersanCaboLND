...
coches
    //.filter(e => e.year)
    .map( e => e.precio)
    .reduce((acumulador, e) => acumulador + e,0) / coches.length;
    console.log(valor);
...
let arrayMarca = res.get(coches[0].marca);
if (typeof arrayMarca === 'undefined') {
    arrayMarca = [coches[0]];
}
arrayMarca.push(coches[0]);
//res.set(coches[0].marca,cohes[0]);